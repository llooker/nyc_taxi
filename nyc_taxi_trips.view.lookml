...
#   sql_table_name: "[nyc-tlc:yellow.trips],[nyc-tlc:green.trips_2015]"
  ...
  - dimension_group: pickup
    type: time
    timeframes: [time, date, week, month, year, week_of_year, hour_of_day, day_of_week, minute5, minute10, time_of_day]
  ...
  - dimension: rounded_location_valid
    type: yesno
    sql: |
        (round(${TABLE}.pickup_latitude, 3)) > 0.1
        AND 
  ...
  - dimension: passengers_tiered
    sql_case:
      one: ${number_of_passengers} <=1
      two:  ${number_of_passengers} = 2
      three or more: ${number_of_passengers} >= 3
  ...
  - dimension: trip_distance_tiers
    type: tier
    tiers: [0,1,2,3,5,10,20,30,50]
    sql: ${trip_distance}
    
  - dimension: payment_type
    type: string
    sql_case:
      Credit card: ${TABLE}.payment_type='CRD'
      Cash: ${TABLE}.payment_type='CSH'
      No charge: ${TABLE}.payment_type='NOC'
      Dispute: ${TABLE}.payment_type='DIS'
      else: 'Unknown'
